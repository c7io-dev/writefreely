...
func (h oauthHandler) showOauthSignupPage(app *App, w http.ResponseWriter, r *http.Request, tp *oauthSignupPageParams, errMsg error) error {
	...
	hashedPass := []byte{}
	hasPass := false
	// Mastodon handles are [a-z0-9_], so lowercasing and swapping underscores for
	// hyphens lands inside our charset without collapsing two distinct handles onto
	// one name. Anything that still fails validation (leading hyphen, too short,
	// reserved) has no signup form left to correct it on, so fail the signup rather
	// than create a user whose blog is unreachable.
	username := strings.Replace(strings.ToLower(tp.TokenUsername), "_", "-", -1)
	if !author.IsValidUsername(h.Config, username) {
		return impart.HTTPError{Status: http.StatusBadRequest, Message: fmt.Sprintf("Username %q is reserved or isn't valid on this instance.", username)}
	}
	newUser := &User{
		Username:   username,
		HashedPass: hashedPass,
